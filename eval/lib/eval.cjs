...
async function main() {
  ...
  for (const name of docs) {
    ...
    console.log(name.padEnd(22), 'Q', String(s.questions).padStart(3), '| found', pc(s.detectRecall), '| precision', pc(s.detectPrecision), '| location', pc(s.location), '| choice', pc(s.choiceCorrect), s.apiError ? '| ERROR ' + s.apiError.slice(0, 80) : '', mode === 'live' ? '| $' + s.cost.toFixed(4) + ' (' + (s.cost / Math.max(1, s.pages)).toFixed(4) + '/page)' : '');
  }
  ...
  console.log('ALL'.padEnd(22), 'Q', String(results.reduce((m, s) => m + s.questions, 0)).padStart(3), '| found', (tot('detectRecall') * 100).toFixed(1) + '%', '| invented', extra, '| location', (tot('location') * 100).toFixed(1) + '%');
